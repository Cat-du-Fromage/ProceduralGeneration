...
namespace KWZTerrainECS
{
    public partial class UnitSystem : SystemBase
    {
        ...
        public partial struct JMoveUnits : IJobEntity
        {
            ...
            public void Execute(ref Translation position, in DynamicBuffer<BufferPathList> bufferUnitPath)
            {
                int2 mapSizeXY = NumChunkXY * ChunkNumQuadsPerLine;
                int chunkIndex = ChunkIndexFromPosition(position.Value, NumChunkXY, ChunkNumQuadsPerLine);
                ...
                if (bufferUnitPath[0].Value == chunkIndex)
                {
                    ...
                    int indexInChunk = CellChunkIndexFromGridIndex(position.Value, mapSizeXY, ChunkNumQuadsPerLine);
